...
plot_dir = 'test76Plots/' + extension + '/'
array_dir = 'test76arrays/'
fig_specification = ''
...
ax.hist(sm_sample, bins = np.linspace(0, max_bin, nbins), label = 'SM', density = True, alpha = 0.7)
...
def run_toys_luminosity(luminosity, prob_threshold, sm_cross_section, eft_cross_section, detector_efficiency):
    ...
    """ # Samplying Poisson toys method (bumpy with low number of events)
    ...
    """
    # Poisson with floats method
        # Calculate Poisson factor
    poisson_events_from_probs_cut = True
    if poisson_events_from_probs_cut == True:
        n_sm = float_mean_N_sm_after_cut
        n_mixed = float_mean_N_mixed_after_cut
        mu_sm = float_mean_N_sm_after_cut
        mu_mixed = float_mean_N_mixed_after_cut

        n_sm_list = N_sm_after_cut_list
        n_mixed_list = N_mixed_after_cut_list

    elif poisson_events_from_probs_cut != True:
        n_sm = float_mean_N_toy_sm_events
        n_mixed = float_mean_N_toy_mixed_events
        mu_sm = float_mean_N_toy_sm_events
        mu_mixed = float_mean_N_toy_mixed_events

        n_sm_list = N_toy_sm_events_list
        n_mixed_list = N_mixed_sm_events_list

    # Mixed Poisson approach using floats for mean but sampled ints for n
    approach = "pure float"
    if approach == "mixed approach":
    ...
    elif approach == "pure float":
        print("pure appraoch")
        LLR_sm_poisson_list = []
        LLR_mixed_poisson_list = []
        for toy in range(N_toys):
            try:
                LLR_sm_poisson = -2*(n_sm*log(mu_sm/mu_mixed) + (mu_mixed - mu_sm))
                LLR_mixed_poisson = -2*(n_mixed*log(mu_sm/mu_mixed) + (mu_mixed - mu_sm))

                LLR_sm_std = np.sqrt(4 * mu_sm) * np.log(1 + (mu_mixed - mu_sm)/mu_sm)
                LLR_mixed_std = np.sqrt(4 * mu_sm + (mu_mixed - mu_sm)) * np.log(1 + (mu_mixed - mu_sm)/mu_sm)

                # Sample from Gaussian (we assume that the dist becomes Gaus distributed anyway so we can do this)
                gaus0 = np.random.normal(LLR_sm_poisson, LLR_sm_std, 1000)[0] # This is not at all a pretty way of sampling
                gaus1 = np.random.normal(LLR_mixed_poisson, LLR_mixed_std, 1000)[0]
                LLR_sm_poisson_list.append(gaus0)
                LLR_mixed_poisson_list.append(gaus1)
    ...
    print("\nFLOATS", n_sm, n_mixed, mu_sm, mu_mixed, "\n")
    ...
    for i in range(N_toys):
        ...
        # With Poisson floats
        #LLRsm = LLR_sm_poisson + sm_sample_toy_log_likelihoodsm[i]-sm_sample_toy_log_likelihoodeft[i]
        #LLReft = LLR_mixed_poisson + eft_sample_toy_log_likelihoodsm[i]-eft_sample_toy_log_likelihoodeft[i]

        # Poisson only
        #LLRsm = LLR_sm_poisson
        #LLReft = LLR_mixed_poisson
    ...
    print("LLRs",np.average(LLR_sm_poisson), np.average(sm_sample_toy_log_likelihoodsm)-np.average(sm_sample_toy_log_likelihoodeft), "N,b,s",mu_mixed,mu_sm,mu_mixed-mu_sm)
    print("LLRs",np.average(LLR_mixed_poisson), np.average(eft_sample_toy_log_likelihoodsm)-np.average(eft_sample_toy_log_likelihoodeft), "N,b,s",mu_mixed,mu_sm,mu_mixed-mu_sm)
    #print("LLRs",np.average(LLR_sm_poisson), "N,b,s",mu_mixed,mu_sm,mu_mixed-mu_sm)
    #print("LLRs",np.average(LLR_mixed_poisson), "N,b,s",mu_mixed,mu_sm,mu_mixed-mu_sm)
